...
export const metadata: Metadata = {
  ...
  description: "Lead Scout setup and launcher for Sumac Systems.",
  robots: {
    index: false,
    follow: false,
...
function SetupGraphic({ labels }: { labels: string[] }) {
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1.5" aria-hidden="true">
      {labels.map((label, index) => (
        <div key={label} className="contents">
          <div className="flex min-w-0 items-center gap-2 rounded-lg border border-white/[0.1] bg-black/25 px-2 py-2">
          ...
          {index < labels.length - 1 ? (
            <span className="flex items-center justify-center gap-1.5 px-0.5" aria-hidden="true">
              <span className="h-px w-3 bg-sumac-brandy/50 sm:w-5" />
              <span className="h-1.5 w-1.5 rounded-full bg-sumac-brandy/80" />
            </span>
          ) : null}
        </div>
      ))}
  ...
  return (
    <div className="flex min-h-screen flex-col bg-sumac-dark text-white">
      ...
      <main className="flex-1 pt-[var(--site-header-h)]">
        ...
        <section className="relative overflow-hidden border-b border-sumac-brandy/15 px-5 py-12 sm:px-6 md:px-8 md:py-16">
          ...
          <div className="relative mx-auto max-w-5xl">
            <h1 className="max-w-3xl text-4xl font-bold leading-[1.08] tracking-tight sm:text-5xl md:text-6xl">Get Lead Scout running.</h1>
